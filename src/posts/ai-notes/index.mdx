...
```bash
pkgs = "adobe-source-han-sans-cn-fonts adobe-source-han-serif-cn-fonts fcitx 
        kcm-fcitx fcitx-gtk2 fcitx-gtk3 fcitx-qt4 fcitx-qt5 fcitx-ui-light 
        fcitx-table-other"
```
...
### Configuration
Edit `.xinitrc` file. Copy the following to the bottom of file.
```bash
...
Update the system via `sudo pacman -Syu` and check whether fcitx icon appear at the bottom right corner (I'm using KDE). 
...
If it got output... congrats, your Manjaro had detected GPU successfully. 
Otherwise, you need to figured it out by yourself.

For instance, I am using GeForce RTX 3060 (Non-Ti version) on my machine. I need to get the 
latest driver version from the [Unix Driver Archive](https://www.nvidia.com/en-us/drivers/unix/), the
most compatible version for mine is 460.80.
